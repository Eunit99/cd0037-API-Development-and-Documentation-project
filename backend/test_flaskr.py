...
class TriviaTestCase(unittest.TestCase):
    ...
    """@COMPLETED: TODO Write at least one test for each test for successful
    ...
    def test_search_question_not_found(self):
        response = self.client().post(
            "/questions", json={"search_term": "tryHarderAgain"}
        )
        data = json.loads(response.data)
        # gets questions with the same search term as expected output
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(data["questions"]), 0)
        self.assertEqual(data["search_term"], "tryHarderAgain")
        self.assertEqual(data["total_questions"], 0)
        self.assertIn("total_questions", data)
        self.assertIn("search_term", data)
    ...
    def test_get_quizzes(self):
        response = self.client().post(
            "/quizzes", json={"previous_questions": [5], "category": "all"}
        )
        data = json.loads(response.data)
